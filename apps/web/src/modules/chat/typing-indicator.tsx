 ...
 *
 * `aria-live="polite"` con altura reservada: el bloque ocupa su sitio aunque
 * esté vacío, para que la lista de mensajes no dé un salto cada vez que
 * alguien empieza o deja de escribir.
 */
export function TypingIndicator({ users }: { users: PresenceUser[] }) {
  return (
    <p aria-live="polite" className="text-ink-muted h-5 px-4 text-xs">
      {describe(users)}
    </p>
  );
}
